...
- 範例以及說明都是筆者個人見解，因為沒有大量的項目經驗，所以會有知識點的疏漏以及錯誤。
...
- OOP編程中的重要思維方式
  ...
  - 為了交互對象之間的松耦合設計而努力
- 運用設計模式的時機一般來說都是在重構代碼。在項目開發初期，尤其是游戲，需求會經常變化，很難直接運用上設計模式。
...
適配器擔任的責務類似於"代理"，他封裝了其它類，並提供了新的調用方法。適配器包裹了類A，使類A在外部使用時看起來像是另一個類，但核心依然是類A。

在舊時的游戲，比較經典的就是火車下有一個人物的模組，可以理解成他使用了一個火車的適配器包住了人物類，然后覆寫了外觀顯示(Display)，這樣做就是可以覆用人物的移動以及受傷的方法，不需要再重新編寫(猜測)。

特別是項目開發到後期，代碼不好做修改時，適配器模式就非常實用。
...
代理模式能夠自由地切換到任意能代理的對象，令「使用者」與「具體對象」之間解除耦合。

也可以做與第三方庫的隔離。例如Unity中的XML解釋可以使用C#或Mono版本的，可以利用代理模式，將其抽象化，這樣就可以讓項目與具體的第三方庫隔離開來。

## 橋接模式 (Bridge)

> 將抽象部分與它的實現部分分離，使它們都可以獨立地變化。

類似於`策略模式`的進階版，將使用者與被使用者之間的關係抽象化成組合關係，使其不會與具體的類相互依賴，達到解耦效果。
...
與`裝飾模式`的作用相同，可以在不改變元素的前提下，增加元素的功能。每一個訪問者都是一種"算法"。
